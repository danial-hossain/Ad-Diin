...
// ✅ FIX: যেকোনো time format কে "HH:MM" (24-hour) এ convert করে
function toInputTime(timeStr: string): string {
  if (!timeStr) return '';

  // 12-hour format: "5:00 AM" বা "05:00 PM"
  ...
  // 24-hour format with seconds: "05:00:00"
  if (timeStr.includes(':')) {
...
export default function AdminPanel() {
  ...
  // Prayer edit modal state
  const [editingPrayer, setEditingPrayer]   = useState<any | null>(null);
  const [editPrayerTime, setEditPrayerTime] = useState(''); // সবসময় "HH:MM" format
  ...
  // ✅ FIX: prayer_time (raw H:i:s) কে প্রাধান্য দিন, না থাকলে time (formatted) ব্যবহার করুন
  const handleOpenEditPrayer = (prayer: any) => {
    setEditingPrayer(prayer);
    const rawTime = prayer.prayer_time || prayer.time || '';
    setEditPrayerTime(toInputTime(rawTime)); // সবসময় "HH:MM" format এ set করুন
    setEditError('');
  };

  // ✅ FIX: editPrayerTime "HH:MM" format এ আছে, শুধু ":00" যোগ করে backend এ পাঠান
  const handleSavePrayer = async () => {
    if (!editingPrayer) return;
    if (!editPrayerTime) {
      setEditError('সময় দিন');
      return;
    }
    setEditSaving(true);
    setEditError('');
    try {
      const timeToSend = editPrayerTime + ':00'; // "HH:MM" → "HH:MM:00"
      ...
      if (d.success) {
        ...
        // Validation error details দেখান
        if (d.errors?.prayer_time) {
          setEditError(d.errors.prayer_time[0]);
        } else {
          setEditError(d.message || 'Update failed');
        }
      }
    } catch (err) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className={`${card} rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl`}>
            ...
            <div className="mb-4">
              ...
              {/* ✅ FIX: value="HH:MM", onChange এ শুধু "HH:MM" রাখুন */}
              <input
                type="time"
              ...
              <p className={`text-xs mt-1 ${sub}`}>
                বর্তমান সময়: {editingPrayer.time || editingPrayer.prayer_time}
              </p>
            </div>
            ...
            <div className="flex gap-3 justify-end">
              <button
                ...
                className={`px-4 py-2 border ${bdr} rounded-lg text-sm ${text}`}
              >
                বাতিল
              </button>
              <button
                        ...
                        <tbody>
                          {users.filter(u => !searchQuery ||
                            ...
                            <tr key={u.id} className={`border-b ${bdr}`}>
                              ...
                              <td className="p-3">
                                <div className="flex items-center gap-2">
                                  <button onClick={() => handleToggleUser(u.id, !!u.is_active)} className="p-1 rounded hover:bg-gray-100" title={u.is_active ? 'Deactivate' : 'Activate'}>
